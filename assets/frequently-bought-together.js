/**
 * Acharkhastory Frequently Bought Together.
 * Adds the selected main product and companion products through Shopify's AJAX cart endpoint.
 */
(function () {
  'use strict';
  ...
  var setMessage = function (root, message, isError) {
    var node = root.querySelector('[data-acs-fbt-message]');
    if (!node) return;

    node.textContent = message || '';
    node.style.color = isError ? 'var(--acs-color-sale)' : 'var(--acs-color-background)';
  };

  var getItems = function (root) {
    return toArray(root.querySelectorAll('[data-acs-fbt-checkbox]:checked:not(:disabled)'))
      .map(function (checkbox) {
        return {
          id: Number(checkbox.value),
          quantity: 1,
          properties: {
            _bundle: 'Frequently Bought Together',
          },
        };
      })
  ...
  var updateSummary = function (root) {
    var countNode = root.querySelector('[data-acs-fbt-count]');
    var totalNode = root.querySelector('[data-acs-fbt-total]');
    var submit = root.querySelector('[data-acs-fbt-submit]');
    var currency = root.getAttribute('data-currency') || 'INR';
    var total = 0;
    var count = 0;

    toArray(root.querySelectorAll('[data-acs-fbt-item]')).forEach(function (item) {
      var checkbox = item.querySelector('[data-acs-fbt-checkbox]');
  ...
  var syncVariant = function (root, select) {
    var item = select.closest('[data-acs-fbt-item]');
    if (!item) return;

    var selected = select.selectedOptions[0];
    var checkbox = item.querySelector('[data-acs-fbt-checkbox]');
    var price = item.querySelector('[data-acs-fbt-price]');
    var compare = item.querySelector('[data-acs-fbt-compare]');
  ...
  var addSelected = function (root) {
    ...
    var addLabel = root.getAttribute('data-add-label') || 'Add all to bag';
  ...
  var init = function (root) {
    toArray(root.querySelectorAll('[data-acs-fbt-checkbox]')).forEach(function (checkbox) {
    ...
    toArray(root.querySelectorAll('[data-acs-fbt-variant-select]')).forEach(function (select) {
    ...
    root.querySelector('[data-acs-fbt-submit]')?.addEventListener('click', function () {
      addSelected(root);
    });
  ...
  ready(function () {
    toArray(document.querySelectorAll('[data-acs-fbt]')).forEach(init);
  });
})();
